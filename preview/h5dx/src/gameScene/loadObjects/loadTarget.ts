...
export function loadTarget(this: GameSceneLoaders, item: TargetItem): void {
    ...
    if (this.nightLevel) {
        ...
        for (
            let z = GameSceneConstants.IMG_CHAR_ANIMATIONS_SLEEPING_zzz_start;
            z <= GameSceneConstants.IMG_CHAR_ANIMATIONS_SLEEPING_zzz_end;
            z++
        ) {
            zzzFrames.push(z);
        }
        const zzzHold = new Array(15).fill(GameSceneConstants.IMG_CHAR_ANIMATIONS_SLEEPING_zzz_start);
