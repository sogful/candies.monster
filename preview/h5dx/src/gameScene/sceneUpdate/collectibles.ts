...
import { disableGhostCycleForBubble, isGhostBubble } from "./bubbles";
    ...
    if (this.camera.type !== Camera2D.SpeedType.PIXELS || !this.ignoreTouches) {
        for (let i = 0, len = this.stars.length; i < len; i++) {
            ...
            if (s.timeout > 0 && s.time === 0) {
                ...
                let hits = false;
                const canCollect = !this.nightLevel || (s as Star & { isLit?: boolean | null }).isLit === true;
                if (!canCollect) {
                    continue;
                }
                if (this.twoParts !== GameSceneConstants.PartsType.NONE) {
                    hits =
                        (this.candyL && !!GameObject.intersect(this.candyL, s) && !this.noCandyL) ||
                        (this.candyR && !!GameObject.intersect(this.candyR, s) && !this.noCandyR);
                } else {
