...
class JsonLoader {
    ...
    async start(): Promise<void> {
        ...
        try {
            ...
            const customLevelOverride = getCustomLevel();
            let filesToLoad = levelFiles;

            if (customLevelOverride) {
                // Skip all official level files when custom level is present
                const skippedFiles = levelFiles.length;
                this.totalJsonFiles -= skippedFiles;
                this.progressCallback?.(this.loadedJsonFiles, this.totalJsonFiles);

                filesToLoad = [];
                this.jsonCache.set(CUSTOM_LEVEL_CACHE_KEY, customLevelOverride);
            }

            // Load all remaining level JSON files
            const promises = filesToLoad.map(async ({ url, key }) => {
                try {
